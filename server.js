...
io.on('connection', function(socket){
    ...
    socket.on('play_token', function(payload){
        log("Play token with", JSON.stringify(payload));
        if(('undefined' === typeof payload || !payload)){
        ...
        if((game.whose_turn === 'white') && (game.player_white.socket != socket.id) || (game.whose_turn === 'black') && (game.player_black.socket != socket.id) ){
            var error_message = 'play_token message nplayed out of turn';
            log(error_message);
            socket.emit('play_token_response', {
                                                result: 'fail',
                                                message: error_message
...
function calculate_valid_moves(who, board){
    ...
    for(row = 0; row < 8; row++){
        for(column=0; column<8; column++){
            if(board[row][column]===' '){
                northwest = valid_move(who, -1, -1, row, column, board);
                north = valid_move(who, -1, 0, row, column, board);
                northeast = valid_move(who, -1, 1, row, column, board);

                east= valid_move(who, 0, 1, row, column, board);
                west= valid_move(who, 0, -1, row, column, board);

                southwest = valid_move(who, 1, -1, row, column, board);
                south = valid_move(who, 1, 0, row, column, board);
                southeast = valid_move(who, 1, 1, row, column, board);

                if(northeast || north || northeast || east || west || south || southwest || southeast){
